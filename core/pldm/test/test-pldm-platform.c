...
static int pldm_test_sensor_event_handle(uint8_t *event_data, size_t event_data_length)
{
	...
	printf("%s: PLDM_SENSOR_EVENT sensor_id = %d | event_state =%d \n",__func__, (int)sensor_id, (int)event_state);
		...
		void **response_msg, size_t *response_len)
{
	...
	case PLDM_PLATFORM_EVENT_MESSAGE:
		...
		case PLDM_PDR_REPOSITORY_CHG_EVENT:
			printf("ABHISHEK: EVENT %d PLDM_PDR_REPOSITORY_CHG_EVENT || event_data_offset= %d \n", (int)event_class, (int)event_data_offset);
			if (platform_special_case == PLATFORM_EVENT_ERROR)
				completion_code = PLDM_ERROR;
			break;

		case PLDM_SENSOR_EVENT:
			printf("ABHISHEK: EVENT %d PLDM_SENSOR_EVENT\n", (int)event_class);
	...
	case PLDM_SET_STATE_EFFECTER_STATES:
		...
		rc = decode_set_state_effecter_states_req(request_msg, payload_len,
							...
							&comp_effecter_count,
							&field);
		printf("ABHISHEK %s test: PLDM_SET_STATE_EFFECTER_STATES: effecter_id = %d || %d   iFIELD %x  ||state %x\n", __func__,(int)effecter_id,comp_effecter_count, (int)field.set_request, field.effecter_state);
		if (rc != PLDM_SUCCESS)
			return OPAL_PARAMETER;
...
int main(void)
{
	...
	do {
		...
		printf("PLDM PLARFORM TEST :%s PASSED\n", tc->name);
	} while ((++tc)->fn);
