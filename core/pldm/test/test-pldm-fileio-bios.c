			...
			void **response_msg, size_t *response_len)
{
	...
	printf("%s function test called\n", __func__);
	if (((struct pldm_msg *)request_msg)->hdr.type
			!= PLDM_OEM)
...
static int test_fileio_init(void)
{
	...
	if (rc != OPAL_SUCCESS) {
		printf("PLDM_TEST: %s failed rc = %d\n", __func__, rc);
		return rc;
	}
	return OPAL_SUCCESS;
}
...
static int test_read_after_init_normal_case(void)
{
	...
	rc = pldm_file_io_read_file(TEST_FILE_IO_HANDLE, TEST_FILE_IO_LENGTH, 0, buf_read, size);
	if (rc != OPAL_SUCCESS) {
		printf("PLDM_TEST: %s failed :: pldm_file_io_read_file rc = %d\n", __func__, rc);
		return OPAL_PARAMETER;
	}
